...
Section Translate.
  ...
  Definition binded_funcall (funid:ident)(e:exp ) : Clight.statement :=
    match  get_pou_name (typeof e) with 
    | Some po_nm =>
        funcall  (prefix funid po_nm) [Clight.Eaddrof (gen_exp e) (type_of_inst_p po_nm)]
    | None => Clight.Sskip
    end.
  ...
  Fixpoint translate_stmt (s: stmt) : Clight.statement :=
    ...
    | Repeat s e =>
      Clight.Sloop  (translate_stmt s) (Clight.Sifthenelse (gen_exp e) Clight.Sskip Clight.Sbreak)
    | Return =>
      Clight.Sreturn None
    | Exit =>
      Clight.Sbreak
    ...
    | Call e => binded_funcall(ID_EXEC) e
      
    | Reset e  => binded_funcall(ID_RESET) e
    | _ => Clight.Sskip
    end.
...
Definition fundef
  ...
  Definition mk_members (p_decl: pou_decl): Ctypes.members :=
    map translate_param p_decl.(pou_vars) .
  Definition mk_members_temp (p_decl: pou_decl): Ctypes.members :=
  map translate_param p_decl.(pou_vars) .
   (* Definition mk_members(p_decl: pou_decl):list (ident * Ctypes.type) :=
    (map mk_members  p_decl.(pou_vars)) ++ (map mk_members  p_decl.(pou_temps) ) . *)
  Definition make_struct (p_decl: pou_decl)
  :  (Ctypes.composite_definition) :=
  Ctypes.Composite (pou_name p_decl) Ctypes.Struct (mk_members p_decl) Ctypes.noattr .
  ...
  let reset_fun := fundef ([(self ,  (type_of_inst_p p_name) )]) [] (mk_members_temp p_decl) Ctypes.Tvoid reset_body   in
  ...
  let exec_fun := fundef ([ (self ,  (type_of_inst_p p_name)) ]) [] (mk_members_temp p_decl) Ctypes.Tvoid  exec_fun_body  in
  ...
  let composite_definition := make_struct p_decl in
...
Definition check_size (env: Ctypes.composite_env) (id: AST.ident) :=
  ...
  | Some co =>
      ...
      then OK tt
      else Error [MSG "structure is too big: '"; CTX id; MSG "'." ]
  | None =>
      Error [MSG "structure is undefined: '"; CTX id; MSG "'." ]
  end.
