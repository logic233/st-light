...
Module Type ST_SYNTAX
  ...
  | If: exp -> stmt -> stmt -> stmt (*if e then s1 lese s2*)
  | Repeat : stmt -> exp -> stmt    (*repeate s until e*)
  | Return : stmt 
  | Exit : stmt
  | Continue : stmt 
  ...
  Record pou_decl : Type :=
    mk_pou {
        ...
        pou_nodup : NoDup (map fst pou_vars ++ map fst pou_temps);
...
Qed.
  ...
  Definition get_class_by_type (ty : type)(pro:project) : option pou_decl :=
    match get_pou_name ty with
      | None => None
      | Some po_name => find_pou_decl po_name pro
    end.
