...
Proof.
  intros * Occurs.
  induction s; inversion_clear Occurs as [|? ? ? [Hs1|Hs2]];
    split; constructor; ((left; now apply IHs1) || (right; now apply IHs2) || idtac).
  - left; auto.
  - right; auto.
Qed.
Hint Resolve  occurs_in_comp.
...
Lemma staterep_extract'': 
  ...
  Section MatchStates.
    ...
    (*****************************************************************)
    (** we work under a caller method of an po class,            **)
    ...
    Section ExprCorrectness.
      ...
      Section ExpField.
      (* Taking Lemma 5.1.8 Invariants Ensure Semantic Consistency from the paper as an example, 
      this explains the fundamental approach of using theorem proving techniques to verify code generation correctness.*)
        ...
        + (*Var i t*)
          ...
          edestruct make_members_co as (? & Hco & ? & Eq & ? & ?). eauto. apply Findpou.
        ...
        + (*(Field ex i t)*)
          ...
          eapply make_members_co in Findpou'  as (? & Hco & ? & Eq & ? & ?); eauto.
          eapply filter_val_filter_In2,c_objs_field_offset in H3; eauto.
          assert(Hoff: exists d,field_offset tge i ( mk_members_all po0) = Errors.OK d) by eauto .
    ...
    Section AssignCorrectness.
      ...
      Variable (ex  : expr) (ty : type) (v : val)
               (ae : expr)(x : ident).

      Hypothesis (WTv : wt_val v ty)
                 (Teq : Clight.typeof ae = cltype ty)
                 (Hin : In (x, ty) (po.(pou_temps)))
                 (Hty : is_inst ty = false).

      (** ex = ae : x can be a temp or an indirect access field (output variable) *)

      Lemma exec_assign:
        forall m1 le1 ve1 P1,
          m1 |=  varsrep po ve1 le1
                ** P1 ->
          ...
          exists le',
            exec_stmt_fe function_entry2 tge e le1 m1
                      (Clight.Sset x  ae)
                      E0 le' m1 Out_normal
            /\ m1 |=  varsrep po (Env.add x v ve1) le'
                     ** P1
      ...
      Proof.
        ...
        assert (x <> self). 
        {
          intro; subst;  eapply (po_notreserved self); eauto.
          assert(In (self, ty) ((pou_vars po) ++ (pou_temps po)  )).
          eapply in_or_app ; eauto.
          eapply (In_InMembers self ty);eauto.
        }

        exists ((PTree.set x v le4)); rewrite PTree.gso; intuition; auto.
        +
        unfold exec_stmt_fe.
        eapply exec_Sset in Ev;eauto.
        + 
        rewrite varsrep_add in Hmem;eauto.
      Qed.
      ...
      Corollary exec_assign_match_states:
          ...
          exists le',
            exec_stmt_fe function_entry2 tge e le m
                      (Clight.Sset x  ae)
                      E0 le' m Out_normal
            /\ m |= match_states gcenv pro po (me, Env.add x v ve) (e, le') sb sofs 
                     ** P .
      Proof.
        intros.
        apply match_states_conj in Hmem as (Hmem & ?).
        eapply sep_swap in Hmem.
        assert (A1:m |= staterep gcenv pro (pou_name po) me sb (Ptrofs.unsigned sofs) ** P)
          by (eapply sep_proj2;eauto).
        eapply sep_drop2 in Hmem.
        eapply exec_assign in Hmem as ( le'& ? & ? & ?);eauto.
        exists le';intuition; auto.
        apply match_states_conj; intuition;eauto.
        unfold varsrep in H2.
        assert(A2: Forall (match_var (Env.add x v ve) le') (map fst (filter_val (pou_temps po))) /\ 
              m |= staterep gcenv pro (pou_name po) me sb (Ptrofs.unsigned sofs) ** P
        ).
        {
          split;eauto.
          apply sep_pick1 in H2.
          apply sep_pure';eauto.
        }
        eapply  sep_pure in A2.
        eapply sep_swap;eauto.
        eapply wt_state_add;eauto.
        eapply NoDupMembers_filter,pou_nodup_temps;eauto. 
      Qed.


    End AssignCorrectness. 

    Section AssignCorrectness_var.
      ...
      Hypothesis (WTv : wt_val v ty)
                 (Teq : Clight.typeof ae = cltype ty)
                 (Hin : In (x, ty) (po.(pou_vars)))
                 (Hty : is_inst ty = false).

      (** ex = ae : x can be a temp or an indirect access field (output variable) *)

      (* Lemma exec_assign_var:
        forall m1 le1 ve1 P1,
          m1 |=  varsrep po ve1 le1
                ** P1 ->
          eval_expr tge e le1 m1 ae v ->
      ...
      Corollary exec_assign_match_states__:
          eval_expr tge e le m ae v ->
          exists m',
            exec_stmt_fe function_entry2 tge e le m
                      (Clight.Sassign (deref_field self po.(pou_name) x (cltype ty))  ae)
                      E0 le m' Out_normal
            /\ m' |= match_states gcenv pro po (add_val x v me, ve) (e, le) sb sofs 
                     ** P .
      Proof.
        intros.
        edestruct evall_self_field as (?&?& Hofs' &?);eauto.
        eapply find_pou_decl2find_pou in Findpou.
        inv Findpou.
        edestruct match_states_assign_state_mem;eauto.
        exists x2.
        inv H2. destruct H3 as (? & ? & ? & ?);eauto.
        split;eauto.
...
(*
    Section FuncallAssignCorrectness.
      ...
      Hypothesis (Hmem1 : m1 |= fieldsrep gcenv ve_callee (co_members instco) instb
                                ** outputrep gcenv po caller ve le outb_co
        ...
        forall outs xs vs,
          ...
          exists m' le',
            exec_stmt_fe function_entry2 tge e le m1
                      (funcall_assign po caller xs o tyo outs)
                      E0 le' m' Out_normal
            /\ m' |= fieldsrep gcenv ve_callee (co_members instco) instb
                    ** outputrep gcenv po caller (Env.adds xs vs ve) le' outb_co
      ...
      Corollary exec_funcall_assign:
        forall xs vs,
          ...
          exists m' le',
            exec_stmt_fe function_entry2 tge e le m1
                      (funcall_assign po caller xs o tyo callee.(m_out))
                      E0 le' m' Out_normal
            /\ m' |= fieldsrep gcenv ve_callee (co_members instco) instb
                    ** outputrep gcenv po caller (Env.adds xs vs ve) le' outb_co
                    ** varsrep caller (Env.adds xs vs ve) le'
    ...
    Lemma exec_binded_funcall:
      forall ys o es me_o' vs rvs cid c prog'' fid callee,
        ...
        exists m' le',
          exec_stmt_fe function_entry2 tge e le m
                    (binded_funcall (rev prog) po caller ys cid o fid es)
                    E0 le' m' Out_normal /\
          m' |= match_states gcenv prog po caller
    ...
    Proof.
      ...
      pose proof Findchild';
        apply find_class_rev in Findchild' as (po & Findchild'); auto.
...
(*
  ...
  Hypothesis SpecIO :
    forall ome ome' poid f vos rvos,
      Forall (fun vo => vo <> None) vos ->
      stmt_call_eval prog ome poid f vos ome' rvos ->
      Forall (fun x => x <> None) rvos.

  Hypothesis NO_NAKED_VARS: Forall_methods (fun m => No_Naked_Vars m.(m_body)) prog.
*)

Lemma exp_eval_no_inst :
forall pro po me ve e v,
wt_exp pro po e ->
exp_eval me ve e (Some v) ->
is_inst (typeof e) = false.
Proof.
  intros.
  induction H0;eauto;simpl.
  + 
    destruct c;eauto.
  +
    inv H.
    unfold type_unop in H4.
    destruct op.
    destruct u;simpl in H4 ; eauto.
    inv H4;eauto.
    destruct (  Cop.classify_notint(cltype (typeof exp0))); inv H4;eauto.
    destruct (  Cop.classify_neg (cltype (typeof exp0))); inv H4;eauto.
    destruct (  Cop.classify_neg (cltype (typeof exp0))); inv H4;eauto.
    apply IHexp_eval in H6.
    destruct (  Ctyping.check_cast (cltype (typeof exp0))(cltype t)) eqn:eq.
    destruct ((typeof exp0));destruct t;simpl in H4; inv H4;eauto ; inv eq.
    inv H4.
  +
    inv H.
    apply IHexp_eval1 in H6.
    apply IHexp_eval2 in H7.
    unfold type_binop in H4.
    destruct (       binop_always_returns_bool op
    || is_bool_binop op &&
       (is_bool_type (typeof e1) &&
        is_bool_type (typeof e2)));inv H4;eauto.

    destruct (  Ctyping.type_binop op (cltype (typeof e1)) (cltype (typeof e2))) eqn:eq.
    destruct t;inv H1;eauto.
    destruct op;
    destruct ((cltype (typeof e1))) ; destruct ((cltype (typeof e2))) ;
    try destruct i0 ; try destruct s ;
    try destruct i1 ; try destruct s0;
...
Check exec_stmt_fe function_entry2. 
  Theorem mutual_correctness:
    (forall p me ve s me' ve'  po    m e le  sb sofs  po_id P pro',
        suffix p pro ->
        stmt_eval p me ve s (me', ve') ->
        find_pou_decl po_id pro = Some po ->
        find_pou po_id pro = Some (po, pro') ->
        m |= match_states gcenv pro po (me, ve) (e, le) sb sofs 
             ** P ->
        exists m' le',
          exec_stmt_fe function_entry2 tge e le m
                    (translate_stmt po s)
  ...
  Check wt_stmt.
    ...
    (* cut ((forall p me ve s (me_ve': menv * venv) prog,
             stmt_eval p me ve s me_ve' ->
             suffix p prog ->
             forall  po prog'  m e le outb_co sb sofs P,
               let (me', ve') := me_ve' in
               forall 
                 (Hmem       :         m |= match_states gcenv prog po (me, ve) (e, le) sb sofs outb_co ** P)
                 (WTs        : wt_stmt prog' po s)
                 ,
               exists m' le',
                 exec_stmt_fe function_entry2 tge e le m
                        (translate_stmt po s) E0 le' m' Out_normal
                 /\ m' |= match_states gcenv pro po (me', ve') (e, le') sb sofs outb_co ** P)
         ).
    { 
      intros Hstmt.
    ...
    intros.
    assert (WTs        : wt_stmt pro po s) by admit.
    induction H0;simpl.
    ...
    + (* (Assign le0 e0) *) 
      ...
      admit.
      (* exists m,(Env.add i (gen_exp po e0) le). *)
    ...
    - inv H0.
      + 
      assert (In (x, ty) (filter_val (pou_vars po))) . 
      {
         apply filter_val_filter_In1.
         eapply exp_eval_no_inst in H8;eauto.
         eapply implicit_cast_keep_inst in A3;eauto.
         inv A1;eauto.
      }
      assert (wt_val v (typeof e0)) .
      {
        apply match_states_conj in H3 as (Hmem &?&?&?).
        eapply pres_sem_exp'; eauto.
      }
      assert (wt_val v ty) by (eapply implicit_cast_keep_wt_val;eauto).
      edestruct evall_self_field as (?&?& Hofs' &?); eauto.
      take (In _ (filter_val (pou_vars po))) and 
      edestruct match_states_assign_state_mem with (4:=it) as (m' & ? & Hofs & ? & Hmem'); eauto.

      rewrite Hofs in Hofs'; inv Hofs'.
      exists m', le; intuition.
      econstructor; eauto using expr_correct; simpl; eauto.
        ++ 
        assert (Clight.typeof (gen_exp po e0) = (cltype (typeof e0)) ) by apply type_pres .
        rewrite H7.
        eapply implicit_cast_keep_value;eauto.
        ++ 
        rewrite Ptrofs.add_unsigned, Ptrofs.unsigned_repr; auto.
     +
     assert (In (x, ty) (filter_val (pou_temps po))) . 
     {
        apply filter_val_filter_In1.
        eapply exp_eval_no_inst in H8;eauto.
        eapply implicit_cast_keep_inst in A3;eauto.
     ...
     assert (wt_val v ty) by (eapply implicit_cast_keep_wt_val;eauto).
     simpl.
    exists m, (PTree.set x v le).
    econstructor;intuition.
    constructor;eauto using expr_correct.
    apply match_states_conj in H3 as (Hmem &?&?&?).
    eapply match_states_conj .
    split.
    ...
    admit.
    rewrite <- H7.
    eapply PTree.gso;eauto.
    assert(self <> x) by admit;eauto.
    + admit.
    (* x = e *)
    - admit.
    ...
    - take (stmt_call_eval _ _ _ _ _ _ _) and rename it into Ev.
      ...
      assert (wt_mem (instance_match o me) p' po).
    ...
    - assert (Cop.bool_val v (Clight.typeof (gen_exp po caller cond)) m = Some b)
    ...
    - eapply find_class_sub_same in Findcl; eauto.
      ...
      assert (wt_stmt prog' (c_objs po) (c_mems po) (meth_vars fm) (m_body fm))
      ...
      assert (fn_body fd = return_with (translate_stmt (rev prog) po fm (m_body fm))
                                       (case_out fm
                                                 None
...
(*
  ...
  (*****************************************************************)
  (** ST_Correctness of the whole program                            **)
  (*****************************************************************)
  ...
  Section dostep.
    ...
    Section MainCorrectness.
      ...
      (*****************************************************************)
      (** ST_Correctness of dostep relatively to loop_call               **)
      (*****************************************************************)
