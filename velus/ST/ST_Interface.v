...
Module Export Op <: ST_OPERATORS.
  ...
  (* Definition implicit_cast(ty1 : type)(ty2 : type) : Prop :=
    match ty1,ty2 with 
    | Tint i1 s1 , Tint i2 s2 =>  i1 = i2 /\ s1 = s2
    | _ , _ => False 
    end. *)
  ...
  .
  Definition implicit_cast:= implicit_cast'.
...
(* 
  ...
  Proof.
    ...
    destruct i; auto.
  Qed. *)
  Lemma implicit_cast_keep_wt_val:
    forall ty1 ty2 v  ,
      implicit_cast ty1 ty2 ->
      wt_val v ty1 ->
      wt_val v ty2 .
  ...
  Lemma implicit_cast_keep_value:
    forall ty1 ty2 v m ,
      ...
      (*[Vundef]*)
      (* v <> Values.Vundef ->  *)
      Cop.sem_cast v (cltype ty1) (cltype ty2) m = Some v.
  Proof.
    intros.
    inv H.
    inv H0;eauto.
    assert(A3:  (n = Int.zero \/ n = Int.one) -> (if Int.eq n Int.zero then Int.zero else Int.one) =n) . 
    {
      intros.
      destruct H0; subst;eauto.
    }
    destruct sz ; destruct sg;eauto;
    unfold Cop.sem_cast;simpl; destruct  (Archi.ptr64)  eqn:eptr ; eauto;simpl;
    apply f_equal; apply f_equal; eauto.
    (* [Vundef] *)
    (* eauto.
    contradiction H1;eauto. *)
  Qed.

  Lemma implicit_cast_keep_inst:
  forall ty1 ty2  ,
  implicit_cast ty1 ty2 ->
  is_inst ty1  =  is_inst ty2.
  Proof.
    intros.
    inv H;eauto.
  Qed.
  ...
  Proof.
    ...
    destruct ty;try(destruct i); simpl;auto.
  Qed. *)
  ...
  Proof.
    ...
    destruct ty, ty'; simpl; GoalMatchMatch; try (contradiction).
  Qed. *)
  ...
  Proof.
    ...
    - destruct u, ty; try destruct i; try destruct s; try destruct f;
        simpl in *;auto. now DestructCases.
