...
Lemma NoDupMembers_make_members:
  forall c, NoDupMembers (mk_members_all c).
Proof.
  intro; unfold mk_members_all.
  pose proof (pou_nodupvars_vals c) as Nodup.
...
Corollary types_pres:
  forall f c caller es,
    ...
    type_of_params (map translate_param f.(m_in)) =
    list_type_to_typelist (map Clight.typeof (map (gen_exp c caller) es)).
Proof.
  intros * Hes.
  apply types_pres'.
...
Lemma c_objs_field_offset:
  forall ge o c po,
    ...
    exists d, field_offset ge o (mk_members_all po) = Errors.OK d.
Proof.
  intros * Hin'.
  unfold field_offset.
  cut (forall ofs, exists d,
            field_offset_rec ge o (mk_members_all po) ofs = Errors.OK d); auto.
  unfold mk_members_all.
  assert(Hin:In (o, c)  (pou_vars po)) by (apply filter_In  in Hin' as (?&?);eauto).
  ...
  rewrite map_cons.
  rewrite <- app_assoc.
  generalize (map mk_members ws ).
  generalize (map mk_members xs ).
  generalize (map mk_members (filter_val (pou_vars po)) ++ map mk_members (pou_temps po) ).
  clear Hin ws xs.
  intros l ws xs.
  induction xs as [|x xs IH]; intros ofs.
  - simpl. destruct (AST.ident_eq o o);eauto.
    contradiction .
  - destruct x as (x, ty).
...
Lemma field_translate_mem_type:
  forall prog ponm po prog',
    find_class ponm prog = Some (po, prog') ->
    forall x ty,
      In (x, ty) po.(c_mems) ->
      field_type x (make_members po) = Errors.OK (cltype ty).
Proof.
  intros * Hfind ? ? Hin.
...
Lemma field_translate_obj_type:
  forall prog ponm po id,
    ...
    forall o c,
      In (o, c) (filter_inst po.(pou_vars) ++  (filter_inst po.(pou_temps))) ->
      (get_pou_name c) = Some id ->
      field_type o (mk_members_all po) = Errors.OK (type_of_inst id).
Proof.
  intros * Hfind ? ? Hin HN.
  apply in_field_type; auto.
...
Section MethodSpec.
  ...
  Proof.
    ...
    - assert (wt_program prog') by (eapply wt_program_find_class; eauto).
      assert (wt_program prog'') by (eapply wt_program_find_class; eauto).
      assert (find_class i prog' = Some (po, p')) as Find1 by (eapply find_class_chained; eauto).
      assert (find_class i prog = Some (po, p')) as Find2 by (eapply find_class_chained; eauto).
...
Ltac inv_trans_tac H En Estep Ereset s f E :=
  match type of H with
    ...
    destruct (find_class n p) as [(c, po)|] eqn: En; try discriminate;
  ...
  Section ClassProperties.
    ...
    Proof.
      ...
      (*  Ctypes.Composite (pou_name p_decl) Ctypes.Struct _members Ctypes.noattr *)
       ...
       assert (In (Composite (pou_name po) Struct (mk_members_all  po) noattr) (concat s)).
      { unfold gen_pou  in E.
        apply split_map in E.
        destruct E as [Structs].
        unfold mk_members_all in Structs.
        assert (Findpou': In po pro ). {
          eapply find_pou_decl_In;eauto.
        }
        apply in_map with (f:=fun p_decl : pou_decl =>[gen_pou_struct p_decl])
          in Findpou'.
        apply in_concat' with (l:=[gen_pou_struct po] ).
        - apply in_eq.
        - now rewrite Structs.
      }
    ...
    Section MethodProperties.
      ...
      Section OutStruct.
        ...
        Lemma global_out_struct:
          exists co,
            gcenv ! (prefix_fun ownerid callerid) = Some copl in SIZE
            /\ co.(co_su) = Struct
      ...
      Proof.
        ...
        - destruct_list l.
          ...
          + destruct (M.E.eq_dec (i0, i1) (o, fid)) as [[E1 E2]|E1]; simpl in *.
            *{ subst.
               apply MapsTo_add_same in Hin; subst.
               exists po, p', fm; repeat split; auto.
...
(*
  ...
  Proof.
    ...
    edestruct well_formed_instance_methods as (c & po & callee & Findc & Findcallee & Notnil); eauto.
